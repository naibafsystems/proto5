$(function () {
    ...
    $('#agregar_comple').on('blur', 'input[type=text],select', function(event) {
        ...
        $('#direccion_clase2').val($.trim(comple));
    });

    $('#frmUbicacion').validate({
        errorClass: 'error-form',
        rules: {
            direccion_clase2: {required: true}
        },
        messages: {
            direccion_clase2: {required: 'Error: Seleccione los complementos que conforman la dirección.' }
        },
        /*acc*/
    ...
    $('#btnSiguiente').on('click', function() {
        ...
        if ($('#frmUbicacion').valid() == true) {
            ...
            if(msgError.length > 0) {
                ...
                validator.showErrors({'direccion_clase2': msgError});
                return false;
            }
