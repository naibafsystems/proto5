$(function () {
    ...
    $('input[type=radio][name=dias_no_comio]').on('change', function() {
        if($(this).val() == 1) {
            $('#dias_no_comio-1-panel').removeClass('hidden');
        } else if($(this).val() == 2) {
            $('#dias_no_comio-1-panel').addClass('hidden');
            $('#numero_no_comio').val('');
        }
    });
    ...
    $('#frmPersona').validate({
        ...
        rules: {
            dias_no_comio: {required: true},
            numero_no_comio: {selectVacio: true}
        },
        messages: {
            dias_no_comio: {required: 'ERROR: Debe seleccionar si la persona por falda de dinero no consumió ninguna de las tres comidas basicas.'},
            numero_no_comio: {selectVacio: 'ERROR: Debe seleccionar cuántos días la persona por falda de dinero no consumió ninguna de las tres comidas basicas.'}
        },
        /*acc*/
