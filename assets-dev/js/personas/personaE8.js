$(function () {
    ...
    $('input[type=radio][name=afiliado_salud]').on('change', function() {
        if($(this).val() == 1) {
            $('#afiliado_salud-1-panel').removeClass('hidden');
        } else if($(this).val() == 2) {
            $('#afiliado_salud-1-panel').addClass('hidden');
            $('input[name=regimen_salud]').prop('checked', false);
        }
    });
    ...
    $('#frmPersona').validate({
        ...
        rules: {
            afiliado_salud: {required: true},
            regimen_salud: {required: true}
        },
        messages: {
            afiliado_salud: {required: 'ERROR: Debe seleccionar si la persona se encuentra afiliado(a) a salud.'},
            regimen_salud: {required: 'ERROR: Debe seleccionar el régimen en el que se encuentra afiliado(a) a salud.'}
        },
        /*acc*/
