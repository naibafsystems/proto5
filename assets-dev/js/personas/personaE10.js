$(function () {
    ...
    $('input[type=radio][name=condicion]').on('change', function() {
        if($(this).val() == 1) {
            $('#limitacion_oir-col').removeClass('hidden');
        } else if($(this).val() == 2) {
            $('#limitacion_oir-col').addClass('hidden');
            $('input[name=limitacion_oir]').prop('checked', false);
        }
    });
    ...
    $('#frmPersona').validate({
        ...
        rules: {
            limitacion_oir: {required: true}
        },
        messages: {
            limitacion_oir: {required: 'ERROR: Debe seleccionar si la persona tiene dificultad para oír la voz o los sonidos.'}
        },
        /*acc*/
